    ...
    <div className="flex flex-col gap-1">
      {SERVICE_SECTIONS.map((section) => {
        ...
        return (
          <div
            key={section.key}
            className="flex items-center gap-4 rounded-md border border-border bg-muted/30 px-3 py-2"
          >
                ...
                return (
                  ...
                  >
                    <Checkbox
                      checked={on}
                      onCheckedChange={() =>
                        toggle(section.key, action.key, on)
                      }
                      disabled={disabled}
                      className="size-3"
                    />
...
export default function ManageUsersPage() {
  ...
  const { data: users, isLoading, mutate: mutateUsers } = useSWR<User[]>(
    "/api/auth/users",
    fetcher
  )
    ...
    <div className="flex flex-col gap-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-border">
          <div className="grid grid-cols-[1fr_1fr_auto_auto_auto_auto] items-center gap-4 border-b border-border bg-muted/50 px-4 py-2 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
            <span>User</span>
            <span>Email</span>
            <span>Role</span>
            <span>Access</span>
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map((u) => {
              ...
              return (
                <div
                  key={u.id}
                  className="grid grid-cols-[1fr_1fr_auto_auto_auto_auto] items-center gap-4 border-b border-border px-4 py-3 last:border-b-0"
                >
                  <div className="flex items-center gap-3">
