...
export function LogsContent({ workspaceId }: { workspaceId: string }) {
  ...
  const { data, isLoading } = useSWR<{ data: LogEntry[]; total: number }>(
    workspaceId ? `/api/w/${workspaceId}/logs?${queryParams}` : null,
    fetcher,
  );
    ...
    <div className="flex flex-col gap-6">
      ...
      {isEmpty ? (
        <EmptyStatePlaceholder
          icon={ScrollText}
          title="No logs yet"
          description="Activity logs will appear here once your bots are running and processing tickets."
          actionLabel="Create your first bot"
          actionHref={`/w/${workspaceId}/bots`}
        />
      ) : (
        <>
          {repoFilter && (
