...
Helm chart for deploying the **jiraclaw** (jiraclaw) Next.js application on Kubernetes with PostgreSQL, MinIO, RabbitMQ, and a background worker.
...
helm dependency update ./jiraclaw-chart
...
Update `jiraclaw-chart/.sops.yaml` with your GPG key fingerprint:
...
```bash
./jiraclaw-chart/scripts/generate-secrets.sh > jiraclaw-chart/secrets.yaml
```

Set your production domain (optional):

```bash
YOUR_DOMAIN=app.example.com ./jiraclaw-chart/scripts/generate-secrets.sh > jiraclaw-chart/secrets.yaml
```
...
```bash
sops jiraclaw-chart/secrets.yaml
```
...
| `image.repository`          | Image name             | `jiraclaw`          |
| `image.tag`                 | Image tag              | `""`                |
...
```bash
# Install
helm secrets upgrade --install jiraclaw ./jiraclaw-chart \
  -f jiraclaw-chart/secrets.yaml \
  --set image.tag=v1.0.0 \
  --wait --timeout 5m

# Upgrade (e.g. with new image tag)
helm secrets upgrade jiraclaw ./jiraclaw-chart \
  -f jiraclaw-chart/secrets.yaml \
  --set image.tag=v1.0.1 \
  --wait --timeout 5m
```

Images are pulled from `ghcr.io/<owner>/jiraclaw` (set in GitHub Actions workflow).
...
```bash
kubectl port-forward service/jiraclaw 3000:3000
```
...
```bash
kubectl port-forward service/jiraclaw-minio 9001:9001
```

RabbitMQ management:

```bash
kubectl port-forward service/jiraclaw-rabbitmq 15672:15672
```
...
```bash
# Decrypt to stdout
helm secrets decrypt jiraclaw-chart/secrets.yaml
...
1. **Missing DATABASE_URL** – The app requires `DATABASE_URL` for Prisma. The chart injects it from the PostgreSQL secret's `postgres-url` key. When using `database.existingSecret`, ensure your secret includes `postgres-url` with the full connection string: `postgresql://USER:PASSWORD@HOST:5432/DATABASE` (replace HOST with `<RELEASE>-jiraclaw-chart-postgres` in your namespace).

2. **Pods not ready** – Check that the app pods are running: `kubectl get pods -l app.kubernetes.io/name=jiraclaw-chart`. If they're in CrashLoopBackOff, inspect logs: `kubectl logs -l app.kubernetes.io/component=app --tail=100`
...
4. **DNS / www** – The ingress serves both `jiraclaw.com` and `www.jiraclaw.com`. Ensure your DNS points to the cluster's ingress controller.
